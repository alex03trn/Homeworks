# Pandas Join Homework Project

## Описание проекта
Этот проект демонстрирует работу с **трёх CSV-файлов**:  

1. **sample_metadata.csv** – метаданные образцов  
2. **mass_spec_results.csv** – результаты масс-спектрометрии  
3. **quality_metrics.csv** – данные о качестве  

Скрипт выполняет **четыре типа join-ов** (`inner`, `left`, `right`, `outer`) по колонке `sample_id` и сохраняет результаты в папку `output`.

## Структура проекта

Содержание скрипта main.py
